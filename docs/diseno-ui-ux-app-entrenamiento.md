...
| **Stat-row segmentado**                               | En vez de cards separadas para métricas relacionadas (tiempo/sets/ejercicios), una sola card con columnas divididas por una línea vertical de 1px. Condensa información sin multiplicar componentes.                                                                                                  | Card de rutina destacada (Home), sesión más reciente (Historial).                                               |

> **Nota sobre la racha**: en la primera iteración se probó un tile de bento grande solo para la racha en el Home, pero no se justificaba (un dato chico ocupando un espacio grande). Se resolvió como _badge chico junto al saludo_ (🔥 N días), como ya estaba definido en la sección de micro-interacciones — el bento se reserva para casos donde el tamaño extra realmente comunica jerarquía.
...
| `text-secondary`       | `#9B9BA5`            | Subtítulos, metadata (min, fecha)                            |
...
- **Bottom nav attached**: pegado a los bordes laterales y al borde inferior de la pantalla (no flotante), esquinas redondeadas solo arriba (`24px 24px 0 0`), fondo `bg-base` con `border-top` sutil. El tab activo es una pill amarilla ancha (ícono + label juntos, texto en `text-on-accent`); los tabs inactivos muestran ícono en blanco + label en `text-secondary` debajo. Barra de gesto (`110×4px`, blanco, esquinas redondeadas) al pie, estilo iOS home indicator.
  - _Decisión tomada tras comparar con alternativa flotante/detached (estilo Samsung One UI 8.5)_: se descartó porque reduce área táctil y no resuelve ningún problema real en una app de un solo propósito — el patrón attached da más superficie de toque, más práctico con manos sudadas en el gym.
...
│ │ 5 sem · 4x/sem              │
│ │      52 min │ 6 ejer.       │  ← stat-row segmentado
...
- Cards de rutina muestran duración estimada en minutos (derivada de `RoutineExercise.targetSets` × tiempos promedio, no se ingresa a mano). **Fuera de scope por ahora: estimación de kcal** — requiere datos fisiológicos (peso, RPE) que la app todavía no captura; se retoma cuando exista el módulo de nutrición.
...
│ Sesiones anteriores           │
│ ┌──────────────────────────┐ │
│ │ 🏋 Piernas          45 min│ │  ← filas uniformes, flat
│ │ 🏋 Empuje           50 min│ │
│ └──────────────────────────┘ │
│ [Home][Rutinas][Historial]…  │
└─────────────────────────────┘
...
| ------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `RoutineCard`       | nombre, meta (días/ejercicios o min), variante grid vs. lista | Home, Lista de rutinas                                                    |
...
| `SessionListItem`   | nombre, fecha, duración, ícono                                | Historial (sesiones anteriores)                                           |
